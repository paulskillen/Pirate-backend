...
export class MediaFolderService {
  ...
  async getAll(
    paginate?: MediaFolderPaginateRequest,
    auth?: any,
  ): Promise<MediaFolder[]> {
    const preQuery: any = {
      status: MediaFolderStatus.ACTIVE,
    };
    const query = AppHelper.generateSearchQuery(paginate, ['name'], preQuery);
  ...
  async create(
    input: MediaFolderCreateRequest,
    auth: any,
  ): Promise<MediaFolder[]> {
  ...
  async move(id: string, parentId: string, admin: any): Promise<MediaFolder[]> {
    await this.folderModel
  ...
  async delete(id: string, admin: any): Promise<MediaFolder[]> {
    await this.folderModel
      .findByIdAndUpdate(id, {
        $set: { status: MediaFolderStatus.INACTIVE, deleteByAdmin: admin?._id },
      })
      .exec();
    return this.getAll();
  }
